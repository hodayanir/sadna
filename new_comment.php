...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $courseCode = $_GET["course_id"];
        ...
        $row = mysqli_fetch_assoc($res);       
        $photo = $row['photo'];
        $insert_query_review2 = "INSERT INTO Course_Reviews (photo,courseCode, comment, first_name,last_name, Date, email) VALUES ('$photo','$courseCode', '$comment', '$first_name', '$last_name', '$date', '$email');";
        $res = mysqli_query($link, $insert_query_review2);
        if($res==TRUE){ 
             header("Location: course_details.php?course_id=$courseCode");
        }
        else {
